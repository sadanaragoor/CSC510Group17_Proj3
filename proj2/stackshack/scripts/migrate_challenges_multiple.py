    ...
    with app.app_context():
        ...
        try:
            ...
            if 'daily_bonuses' in inspector.get_table_names():
                columns = [col['name'] for col in inspector.get_columns('daily_bonuses')]
                constraints = inspector.get_unique_constraints('daily_bonuses')
                
                # Check if bonus_date has a unique constraint
                has_unique = any('bonus_date' in str(constraint) for constraint in constraints)
                
                if has_unique:
                    print("\n[+] Removing unique constraint from daily_bonuses.bonus_date...")
                    # Drop the unique constraint (MySQL syntax)
                    try:
                        db.session.execute(text("ALTER TABLE daily_bonuses DROP INDEX bonus_date"))
                    except Exception as e:
                        # Try alternative syntax
                        try:
                            db.session.execute(text("ALTER TABLE daily_bonuses DROP CONSTRAINT bonus_date"))
                        except:
                            print(f"  Note: Could not drop constraint automatically: {e}")
