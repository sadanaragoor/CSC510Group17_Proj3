...
class ShiftService:
    ...
    STATION_ROLES = ["Grill Master", "Assembly", "Counter/Cashier", "Prep", "Floater"]
    ...
    def create_shift(name, start_time, end_time):
        """Create a new shift"""
        shift = Shift(
            name=name, start_time=start_time, end_time=end_time, is_active=True
        )
        db.session.add(shift)
        db.session.commit()
        return shift
    ...
    @staticmethod
    def update_shift(
        shift_id, name=None, start_time=None, end_time=None, is_active=None
    ):
        """Update a shift"""
    ...
    def create_staff_profile(
        user_id, phone=None, position=None, hire_date=None, notes=None
    ):
    ...
    def assign_shift(user_id, shift_id, assignment_date, station_role):
        ...
        existing = ShiftAssignment.query.filter_by(
            user_id=user_id, shift_id=shift_id, date=assignment_date
        ).first()
    ...
    def get_user_upcoming_shifts(user_id, start_date=None):
        ...
        assignments = (
            ShiftAssignment.query.options(joinedload(ShiftAssignment.shift))
            .filter(
                and_(
        ...
        assignments.sort(
            key=lambda a: (a.date, a.shift.start_time if a.shift else time(0, 0))
        )
    ...
    def get_schedule_table(start_date, end_date=None):
        ...
        assignments = (
            ShiftAssignment.query.filter(
                and_(
                    ShiftAssignment.date >= start_date, ShiftAssignment.date <= end_date
                )
            )
            .join(Shift)
            .join(User)
