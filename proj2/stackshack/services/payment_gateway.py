...
class PaymentGatewayService:
    ...
    def process_payment(self, payment_request):
        ...
        if not validation_result["valid"]:
            return self._create_failure_response(
                payment_request, validation_result["reason"]
            )
        ...
        if payment_method == "card":
            ...
            return self._create_failure_response(
                payment_request, "Invalid payment method"
            )
    ...
    def _process_card_payment(self, request):
        ...
        if not self._validate_expiry(expiry_month, expiry_year):
            return self._create_failure_response(
                request, "Card expired or invalid expiry date"
            )
    ...
    def _process_campus_card_payment(self, request):
        ...
        amount = Decimal(
            str(request["amount"])
        )  # Convert to Decimal to match database type
        ...
        campus_card = CampusCard.query.filter_by(
            id=campus_card_id, user_id=request["user_id"]
        ).first()
        ...
        if campus_card.balance < amount:
            return self._create_failure_response(
                request, f"Insufficient balance. Available: ${campus_card.balance}"
            )
    ...
    def _process_wallet_payment(self, request):
        ...
        if self._should_succeed():
            return self._create_success_response(
                request, payment_method="wallet", payment_provider=wallet_provider
            )
        else:
            reasons = [
