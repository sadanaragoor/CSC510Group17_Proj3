...
class GamificationService:
    ...
    def get_user_points(user_id):
        ...
        total = db.session.query(func.sum(PointsTransaction.points)).filter_by(user_id=user_id).scalar()
    ...
    @staticmethod
    def earn_points(event_type, user_id, points, description=None, order_id=None, apply_multiplier=True):
        """
    ...
    def redeem_reward(reward_type, user_id, order_id=None):
        ...
        if current_points < cost:
            return False, f"Insufficient points. Need {cost}, have {current_points}", None
        ...
        return True, f"Successfully redeemed {reward_type}! Your coupon code is: {coupon_code}", coupon_code
    ...
    def apply_coupon(coupon_code, user_id, order):
        ...
        """
        success, message, coupon_dict = GamificationService.validate_coupon(coupon_code, user_id, order.id)
        ...
        if reward_type == "free_topping":
            ...
            if sauces_found:
                discount_description = f"Free premium sauce applied - Removed: {', '.join(sauces_found)}"
            else:
    ...
    def check_and_grant_badges(user_id, order):
        ...
        badges_to_check = [
            ...
            ("veggie_champion", "Veggie Champion", "Ordered 4+ vegetables", "ingredient"),
            ("carnivore_king", "Carnivore King", "Tried all patty options", "ingredient"),
            ("brave_soul", "Brave Soul", "Ordered spiciest ingredients", "ingredient"),
            ("classic_lover", "Classic Lover", "Ordered same burger 5+ times", "behavioral"),
        ]
        ...
        for slug, name, desc, badge_type in badges_to_check:
            ...
            if slug == "sauce_collector":
                ...
                all_sauces = MenuItem.query.filter_by(category="sauce", is_available=True).all()
                user_sauces = set()
                for o in all_orders:
                ...
                veggie_count = sum(1 for item in order_items 
                                 if any(veg in item.name.lower() for veg in ['lettuce', 'tomato', 'onion', 'pickles', 'capsicum']))
                if veggie_count >= 4:
                    earned = True
            
            elif slug == "carnivore_king":
                # Check if tried all patty types
                all_patties = MenuItem.query.filter_by(category="patty", is_available=True).all()
                user_patties = set()
                for o in all_orders:
                    for item in o.items.all():
                        if item.menu_item_id:
                ...
                has_spicy = any(any(keyword in item.name.lower() for keyword in spicy_keywords) 
                              for item in order_items)
                if has_spicy:
                    earned = True
                ...
                burger_name = order.items.first().burger_name if order.items.first() else None
                if burger_name:
                    same_burger_count = sum(1 for o in all_orders 
                                          if o.items.first() and o.items.first().burger_name == burger_name)
            ...
            if earned:
                ...
                if points_reward > 0:
                    GamificationService.earn_points("badge_earned", user_id, points_reward,
                                                   f"Badge earned: {name}", order.id, apply_multiplier=False)
        
        # Check behavioral badges
        behavioral_badges = [
            ("lunch_rush_warrior", "Lunch Rush Warrior", "10 orders between 12-1 PM", 12, 13),
            ...
            ("mystery_box_master", "Mystery Box Master", "5 surprise box orders", None, None),
        ]
        ...
        for slug, name, desc, hour_start, hour_end in behavioral_badges:
            ...
            if slug == "lunch_rush_warrior":
                ...
                surprise_orders = [o for o in all_orders 
                                 if any(item.burger_name and "surprise" in item.burger_name.lower() 
                                       for item in o.items.all())]
    ...
    def process_order_points(order):
        ...
        order_total = float(order.original_total if order.original_total else order.total_price)
        ...
        user = db.session.get(User, user_id)
        multiplier = GamificationService.TIER_MULTIPLIERS.get(user.tier or "Bronze", 1.0)
        ...
        GamificationService.earn_points("purchase", user_id, final_base_points,
                                      f"Purchase: ${order_total:.2f}", order.id, apply_multiplier=False)
    ...
    def check_daily_bonus(user_id, order):
        ...
        for daily_bonus in daily_bonuses:
            ...
            condition_met = GamificationService._check_daily_condition(
                daily_bonus.condition, order_items, order, order_time, order_hour, order_minute
            )
            ...
            if condition_met:
                ...
                # Award points
                GamificationService.earn_points("daily_bonus", user_id, daily_bonus.points_reward,
                                               daily_bonus.description, order.id, apply_multiplier=False)
                completed_bonuses.append(daily_bonus)
        
        if completed_bonuses:
            db.session.commit()
            return True, completed_bonuses[0] if len(completed_bonuses) == 1 else completed_bonuses
        
        return False, None
    
    @staticmethod
    def _check_daily_condition(condition, order_items, order, order_time, order_hour, order_minute):
        ...
        if condition == "keto_bun":
            ...
            return any("sesame" in name or "black sesame" in name for name in order_item_names)
        elif condition == "veggie_bun":
            return any("beetroot" in name or "carrot" in name for name in order_item_names)
        elif condition == "wheat_bun":
            return any("wheat" in name or "honeywheat" in name for name in order_item_names)
        ...
        # Patty conditions
            ...
            patties = menu_items_by_category.get("patty", [])
            return any("low-calorie beef" in name or "mixed veg" in name for name in patties)
        ...
        # Sauce conditions
            ...
            return any("tomato" in name and "sauce" in name for name in order_item_names)
        ...
        # Topping conditions
            ...
            return any("tomato" in name and "sauce" not in name for name in order_item_names)
        ...
        elif condition == "exact_222" and order_hour is not None and order_minute is not None:
    ...
    def check_weekly_challenge(user_id, order):
        ...
        for challenge in challenges:
            ...
            challenge_def = ChallengeService.WEEKLY_CHALLENGES.get(
                next((k for k, v in ChallengeService.WEEKLY_CHALLENGES.items() 
                      if v["condition"] == challenge.condition), None)
            )
            ...
            if progress_updated and progress.progress >= progress.target and not progress.completed:
                progress.completed = True
                progress.completed_at = datetime.utcnow()
                # Award points
                GamificationService.earn_points("weekly_challenge", user_id, challenge.points_reward,
                                               challenge.description, order.id, apply_multiplier=False)
                completed_challenges.append(challenge)
        
        if completed_challenges:
            db.session.commit()
            return True, completed_challenges[0] if len(completed_challenges) == 1 else completed_challenges
        
        db.session.commit()
        return True, None
    
    @staticmethod
    def _update_weekly_progress(challenge, progress, order, user_id, week_start, week_end):
        ...
        if condition.startswith("three_") or condition.startswith("four_") or condition.startswith("five_"):
    ...
    def get_monthly_leaderboard(month=None, year=None, limit=5):
        ...
        top_earners = db.session.query(
            ...
            PointsTransaction, User.id == PointsTransaction.user_id
        ).filter(
            ...
            func.sum(PointsTransaction.points).desc()
        ).limit(limit).all()
