...
class TestShiftServiceComprehensive:
        ...
        with app.app_context():
            ...
            for i in range(3):
                ShiftService.assign_shift(
                    staff.id, shifts[0].id, week_start + timedelta(days=i), "Grill Master"
                )[0]  # Get assignment from tuple
    ...
    def test_get_user_upcoming_shifts(self, app):
        ...
        with app.app_context():
            ...
            upcoming = ShiftService.get_user_upcoming_shifts(staff.id, start_date=start_date)
    ...
    def test_create_shift(self, app):
        ...
        with app.app_context():
            shift = ShiftService.create_shift(
                "Test Shift", time(10, 0), time(12, 0)
            )
