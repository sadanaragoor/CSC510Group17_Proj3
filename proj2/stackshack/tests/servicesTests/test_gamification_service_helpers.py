...
class TestGamificationServiceHelpers:
    ...
    def test_check_daily_condition_bun_conditions(self, app, test_user):
        ...
        with app.app_context():
            ...
            order_item = OrderItem(
                order_id=order.id,
                menu_item_id=keto_bun.id,
                name="Keto Bun",
                price=keto_bun.price,
                quantity=1,
            )
            ...
            result = GamificationService._check_daily_condition(
                "keto_bun", order_items, order, order_time, 12, 0
            )
    ...
    def test_check_daily_condition_cheese_conditions(self, app, test_user):
        ...
        with app.app_context():
            ...
            result = GamificationService._check_daily_condition(
                "cheddar_cheese", order_items, order, order_time, 12, 0
            )
            assert result is True
    ...
    def test_check_daily_condition_sauce_conditions(self, app, test_user):
        ...
        with app.app_context():
            ...
            order_item = OrderItem(
                order_id=order.id,
                menu_item_id=green_sauce.id,
                name="Green Sauce",
                price=green_sauce.price,
                quantity=1,
            )
            db.session.add(order_item)
            db.session.commit()
            ...
            result = GamificationService._check_daily_condition(
                "green_sauce", order_items, order, order_time, 12, 0
            )
            assert result is True
    ...
    def test_check_daily_condition_topping_conditions(self, app, test_user):
        ...
        with app.app_context():
            ...
            result = GamificationService._check_daily_condition(
                "pickles", order_items, order, order_time, 12, 0
            )
            assert result is True
    ...
    def test_check_daily_condition_time_conditions(self, app, test_user):
        ...
        with app.app_context():
            ...
            result = GamificationService._check_daily_condition(
                "before_11am", order_items, order, order_time, 10, 0
            )
            assert result is True
            ...
            result = GamificationService._check_daily_condition(
                "between_2_4pm", order_items, order, order_time, 14, 30
            )
            assert result is True
            ...
            result = GamificationService._check_daily_condition(
                "after_8pm", order_items, order, order_time, 20, 30
            )
            assert result is True
    ...
    def test_update_weekly_progress_bun_challenges(self, app, test_user):
        ...
        with app.app_context():
            ...
            progress = UserChallengeProgress(
                user_id=test_user, challenge_id=challenge.id, progress=0, target=3
            )
