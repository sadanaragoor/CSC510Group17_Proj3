...
class TestGamificationServiceApplyCouponComprehensive:
    ...
    def test_apply_coupon_free_topping(self, app, test_user):
        ...
        with app.app_context():
            topping = MenuItem(name="Lettuce", category="topping", price=Decimal("0.50"), is_available=True)
            ...
            item = OrderItem(order_id=order.id, menu_item_id=topping.id, name="Lettuce", price=Decimal("0.50"), quantity=2)
            db.session.add(item)
            db.session.flush()
    ...
    def test_apply_coupon_free_premium_sauce(self, app, test_user):
        ...
        with app.app_context():
            sauce = MenuItem(name="Green Sauce", category="sauce", price=Decimal("0.15"), is_available=True)
            ...
            item = OrderItem(order_id=order.id, menu_item_id=sauce.id, name="Green Sauce", price=Decimal("0.15"), quantity=1)
            db.session.add(item)
            db.session.flush()
            
            redemption = Redemption(user_id=test_user, reward_type="free_premium_sauce", points_cost=125)
            db.session.add(redemption)
            db.session.flush()
    ...
    def test_apply_coupon_three_dollar_off(self, app, test_user):
        ...
        with app.app_context():
            ...
            redemption = Redemption(user_id=test_user, reward_type="three_dollar_off", points_cost=300)
            db.session.add(redemption)
            db.session.flush()
    ...
    def test_apply_coupon_three_dollar_off_small_order(self, app, test_user):
        ...
        with app.app_context():
            order = Order(user_id=test_user, total_price=Decimal("2.00"), original_total=Decimal("2.00"), status="Pending")
            db.session.add(order)
            db.session.flush()
    ...
    def test_apply_coupon_free_patty_upgrade(self, app, test_user):
        ...
        with app.app_context():
            patty = MenuItem(name="Beef Patty", category="patty", price=Decimal("3.50"), is_available=True)
            ...
            item = OrderItem(order_id=order.id, menu_item_id=patty.id, name="Beef Patty", price=Decimal("3.50"), quantity=1)
            db.session.add(item)
            db.session.flush()
            
            redemption = Redemption(user_id=test_user, reward_type="free_patty_upgrade", points_cost=250)
            db.session.add(redemption)
            db.session.flush()
    ...
    def test_apply_coupon_skip_queue(self, app, test_user):
        ...
        with app.app_context():
            ...
            redemption = Redemption(user_id=test_user, reward_type="skip_queue", points_cost=300)
            db.session.add(redemption)
            db.session.flush()
    ...
    def test_apply_coupon_five_dollar_off(self, app, test_user):
        ...
        with app.app_context():
            ...
            redemption = Redemption(user_id=test_user, reward_type="five_dollar_off", points_cost=500)
            db.session.add(redemption)
            db.session.flush()
    ...
    def test_apply_coupon_five_dollar_off_small_order(self, app, test_user):
        ...
        with app.app_context():
            order = Order(user_id=test_user, total_price=Decimal("3.00"), original_total=Decimal("3.00"), status="Pending")
            db.session.add(order)
            db.session.flush()
