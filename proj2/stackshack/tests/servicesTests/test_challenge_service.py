...
class TestChallengeService:
    ...
    def test_generate_daily_challenges_idempotent(self, app):
        ...
        with app.app_context():
            ...
            count1 = len(DailyBonus.query.filter_by(bonus_date=today, is_active=True).all())
            
            ChallengeService.generate_daily_challenges(today, max_challenges=2)
            count2 = len(DailyBonus.query.filter_by(bonus_date=today, is_active=True).all())
    ...
    def test_generate_weekly_challenges_idempotent(self, app):
        ...
        with app.app_context():
            ...
            count1 = len(WeeklyChallenge.query.filter_by(week_start=week_start, is_active=True).all())
            
            ChallengeService.generate_weekly_challenges(week_start, max_challenges=3)
            count2 = len(WeeklyChallenge.query.filter_by(week_start=week_start, is_active=True).all())
