...
class TestBadgeAwardingComprehensive:
    ...
    def test_badge_sauce_collector(self, app, test_user):
        ...
        with app.app_context():
            ...
            for sauce_name in sauces:
                sauce = MenuItem(name=sauce_name, category="sauce", price=Decimal("0.25"), is_available=True)
    ...
    def test_badge_veggie_champion(self, app, test_user):
        ...
        with app.app_context():
            ...
            for veg_name in veggies:
                veg = MenuItem(name=veg_name, category="topping", price=Decimal("0.50"), is_available=True, is_healthy_choice=True)
                db.session.add(veg)
                db.session.flush()
    ...
    def test_badge_lunch_rush_warrior(self, app, test_user):
        ...
        with app.app_context():
            ...
            last_order = Order.query.filter_by(user_id=test_user).order_by(Order.id.desc()).first()
