...
class TestShiftRoutes:
    ...
    def test_admin_shifts_dashboard_requires_admin(self, client, app, staff_user):
        ...
        assert (
            b"unauthorized" in response.data.lower()
    ...
    def test_assign_shift_post(
        self, client, app, admin_user, staff_user, sample_shifts
    ):
        ...
        with app.app_context():
            assignment = ShiftAssignment.query.filter_by(
                user_id=staff_user, shift_id=sample_shifts[0], date=today
            ).first()
    ...
    def test_staff_shifts_view_displays_assignments(
        self, client, app, staff_user, sample_shifts
    ):
        ...
        assert (
            b"shift" in response.data.lower()
            or b"assignment" in response.data.lower()
    ...
    def test_update_staff_account(self, client, app, admin_user, staff_user):
        ...
        response = client.post(
            ...
            data={"email": "updated@test.com", "position": "Full-time"},
            follow_redirects=True,
        )
