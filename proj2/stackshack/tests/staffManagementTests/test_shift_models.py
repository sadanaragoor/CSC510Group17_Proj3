...
class TestShiftModel:
    ...
    def test_shift_unique_name(self, app):
        ...
        with app.app_context():
            shift1 = Shift(
                name="Test Shift", start_time=time(9, 0), end_time=time(11, 0)
            )
            db.session.add(shift1)
            db.session.commit()

            shift2 = Shift(
                name="Test Shift", start_time=time(12, 0), end_time=time(14, 0)
            )
...
class TestStaffProfileModel:
    ...
    def test_staff_profile_to_dict(self, app, staff_user):
        ...
        with app.app_context():
            profile = StaffProfile(
                user_id=staff_user, phone="123-456-7890", position="Full-time"
            )
            db.session.add(profile)
            db.session.commit()
