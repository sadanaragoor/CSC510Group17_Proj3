...
def surprise():
    ...
    if try_predefined:
        ...
        for burger_def in PREDEFINED_BURGERS:
            ...
            if 'no_preference' in user_tags or burger_tags.intersection(user_tags):
                ...
                for ingredient_name in burger_def['ingredients']:
                    menu_item = MenuItem.query.filter_by(name=ingredient_name, is_available=True).first()
        ...
        if matching_burgers:
            ...
            if bun and patty:
                ...
                if not cheeses:
                    available_cheeses = MenuItem.query.filter_by(is_available=True, category="cheese").all()
                    if available_cheeses:
                        # Filter by user preferences if vegan
                        if hasattr(current_user, 'pref_vegan') and current_user.pref_vegan:
                            healthy_cheeses = [c for c in available_cheeses if c.is_healthy_choice]
                            if healthy_cheeses:
                ...
                if not toppings:
                    available_toppings = MenuItem.query.filter_by(is_available=True, category="topping").all()
                    if available_toppings:
                        toppings.append(random.choice(available_toppings))
                        all_items.append(toppings[0])
                
                if not sauces:
                    available_sauces = MenuItem.query.filter_by(is_available=True, category="sauce").all()
                    if available_sauces:
    ...
    if hasattr(current_user, 'pref_vegan') and current_user.pref_vegan:
        ...
        if not cheeses:
            cheeses = [c for c in MenuItem.query.filter_by(is_available=True, category="cheese").all() if c]

    # Apply gluten-free preference
    if hasattr(current_user, 'pref_gluten_free') and current_user.pref_gluten_free:
        # Filter buns to gluten-free options (keto, beetroot, carrot buns typically)
        gf_buns = [b for b in buns if any(gf_term in (b.name or "").lower() for gf_term in ['keto', 'beetroot', 'carrot', 'gluten'])]
        if gf_buns:
            buns = gf_buns
