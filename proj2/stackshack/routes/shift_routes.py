...
def admin_staff_management():
    ...
    for staff in staff_members:
        ...
        staff_with_profiles.append({"user": staff, "profile": profile})
...
def assign_shift():
    ...
    assignment, message = ShiftService.assign_shift(
        user_id, shift_id, assignment_date, station_role
    )
    if assignment:
        ...
        flash(
            f"Assignment created: {assignment.user.username} assigned to {assignment.shift.name} as {station_role}.",
            "success",
        )
        return redirect(
            url_for("shift.admin_shifts_dashboard", date=week_start.isoformat())
        )
    else:
...
def staff_shifts_view():
    ...
    upcoming_shifts = ShiftService.get_user_upcoming_shifts(
        current_user.id, start_date=start_date
    )

    # Debug: Print assignments to console
    print(
        f"DEBUG: Found {len(upcoming_shifts)} assignments for user {current_user.id} (username: {current_user.username})"
    )
    for a in upcoming_shifts:
        shift_name = a.shift.name if a.shift else "No shift loaded"
        print(
            f"  - Date: {a.date}, Shift ID: {a.shift_id}, Shift: {shift_name}, Role: {a.station_role}"
        )
    ...
    return render_template(
        "shifts/staff_dashboard.html", shifts_by_date=shifts_by_date_sorted
    )
...
def api_get_assignments():
    ...
    assignments = ShiftAssignment.query.filter(
        and_(ShiftAssignment.date >= start_date, ShiftAssignment.date <= end_date)
    ).all()
