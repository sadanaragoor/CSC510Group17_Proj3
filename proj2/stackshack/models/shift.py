...
class StaffProfile(db.Model):
    ...
    position = db.Column(db.String(100), nullable=True)  # e.g., "Part-time", "Full-time"
...
class Shift(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(100), nullable=False, unique=True)  # e.g., "Morning Prep", "Lunch Rush"
    start_time = db.Column(db.Time, nullable=False)  # e.g., 09:00:00
    end_time = db.Column(db.Time, nullable=False)  # e.g., 11:00:00
    is_active = db.Column(db.Boolean, default=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    
    assignments = db.relationship("ShiftAssignment", backref="shift", lazy="dynamic", cascade="all, delete-orphan")
...
class ShiftAssignment(db.Model):
    ...
    station_role = db.Column(db.String(50), nullable=False)  # Grill Master, Assembly, Counter/Cashier, Prep, Floater
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    
    user = db.relationship("User", backref=db.backref("shift_assignments", lazy="dynamic"))
    ...
    def to_dict(self):
        return {
            ...
            "shift_start": self.shift.start_time.strftime("%H:%M") if self.shift and self.shift.start_time else None,
            "shift_end": self.shift.end_time.strftime("%H:%M") if self.shift and self.shift.end_time else None,
